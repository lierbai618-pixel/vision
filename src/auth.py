...
class AuthManager:
    ...
    def _save(self):
        ...
        data = {"users": self.users, "sessions": self.sessions}
    ...
    def login(self, username: str, password: str) -> dict:
        ...
        self.sessions[session_id] = {"username": username, "created_at": time.time(), "last_active": time.time()}
    ...
    def get_user(self, session_id: str) -> dict | None:
        """获取用户信息.
    ...
    def cleanup_sessions(self, max_age: int = 86400) -> int:
        ...
        """
        now = time.time()
        expired = [sid for sid, session in self.sessions.items() if now - session["last_active"] > max_age]
