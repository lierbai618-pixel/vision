...
class Visualizer:
    ...
    def plot_detection_stats(self, results: dict, output_name: str = "detection_stats.png") -> str:
        """绘制检测统计图表.
    ...
    def plot_batch_results(self, batch_results: dict, output_name: str = "batch_stats.png") -> str:
        """绘制批量检测结果统计.
        ...
        axes[0, 0].pie([successful, failed], labels=["成功", "失败"], autopct="%1.1f%%", colors=["#4CAF50", "#F44336"])
        ...
        # 添加数值标签
        for bar, count in zip(bars, counts):
            axes[0, 1].text(bar.get_x() + bar.get_width() / 2, bar.get_height(), str(count), ha="center", va="bottom")
    ...
    def create_detection_summary_image(self, image_path: str, results: dict, output_name: str = "summary.png") -> str:
        """创建检测结果摘要图片.
        ...
        # 添加信息
        y_offset = 30
        cv2.putText(panel, "Detection Summary", (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)

        y_offset += 40
        if "count" in results:
            cv2.putText(
                panel, f"Objects: {results['count']}", (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 1
            )
            y_offset += 30
        ...
        if "class_names" in results:
            ...
            cv2.putText(
                panel, f"Classes: {', '.join(classes)}", (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 1
            )
            y_offset += 30
        ...
        if results.get("confidences"):
            ...
            cv2.putText(
                panel, f"Avg Confidence: {avg_conf:.2%}", (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 1
            )
