            ...
            if cfg["enable_object"]:
                try:
                    ...
                    results = self._det_model(
                        frame, conf=cfg["conf_threshold"], iou=0.5, agnostic_nms=True, imgsz=640, verbose=False
                    )
...
def main():
    ...
    with st.sidebar:
        ...
        model_option = st.selectbox(
            ...
            index=0,
            key="model_option",
            help="custom_items.pt 是你训练的专用模型，精度最高",
        )
        conf_threshold = st.slider(
            "置信度阈值", 0.0, 1.0, 0.35, 0.05, key="conf_threshold", help="降低阈值可检测更多目标，提高阈值可减少误检"
        )
...
def show_mobile_camera(model_option, conf_threshold):
    ...
    with c1:
        enable_object = st.checkbox("🎯 目标检测", True, key="mob_obj")
    with c2:
        enable_face = st.checkbox("👤 人脸识别", True, key="mob_face")
    with c3:
        enable_mask = st.checkbox("😷 口罩检测", False, key="mob_mask")

    # 摄像头选择
    st.radio("📷 摄像头", ["前置摄像头", "后置摄像头"], horizontal=True, key="cam_facing")
        ...
        for f in files:
            ...
            with st.spinner(f"检测 {f.name}..."):
                ...
                try:
                    ...
                    det_model = det.model
                    raw = det_model(
                        result_img, conf=conf_threshold, iou=0.4, agnostic_nms=True, imgsz=960, verbose=False
                    )
                    cn_names = []
                    confs = []
