...
class TestDetectionPipeline:
    ...
    def test_batch_processing_pipeline(self):
        ...
        results = processor.batch_detect_images(test_folder, "test_batch_results", detection_type="object")
    ...
    def test_api_detection(self):
        ...
        try:
            ...
            with open(test_image, "rb") as f:
                files = {"file": f}
                response = requests.post("http://localhost:8000/api/v1/detect", files=files, timeout=30)
...
class TestDatabaseIntegration:
    ...
    def test_detection_storage(self):
        ...
        # 插入检测记录
        result = {"count": 3, "class_names": ["person", "car", "dog"], "confidences": [0.95, 0.87, 0.92]}

        record_id = db.insert_detection(
            user_id="test_user", detection_type="object", image_path="test.jpg", result=result
        )
